...
const char MinuitParameterSet::prtNameChars[] =
  { '+', '-', '*', '>', ',', '(', ')', '[', ']', '\0'};
const char MinuitParameterSet::ntpNameChars[] =
  { '#', '~', 's', '_', '_', '_', '_', '_', '_', '\0'};
...
void MinuitParameterSet::fillNtp( TFile*& ntpFile, TNtupleD*& ntp ) const
{
  ...
  if(0 == ntp)
    ntp = makeNewNtpForOwner(ntpFile);

  if(dbThis)
    cout << "got an ntuple ptr: " << ntp << endl;
  ...
  if(dbThis)
    cout << "filled the array, now putting this into the ntp" << endl;

  ntp->Fill(array);
  if(dbThis)
    cout << "filled into the ntuple, now deleting array" << endl;
