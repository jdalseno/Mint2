...
bool Amplitude::deleteDependants(){
  bool dbThis=false;
  if(dbThis) cout << "Amplitude::deleteDependants()" << endl;
  _init=false;
  this->removeAllFitParDependencies(); // risky
  deleteLineshapes();
  if(0 != _spinFactor) delete _spinFactor;
  _spinFactor=0;
  if(dbThis) cout << "Amplitude::deleteDependants() done" << endl;
  return true;
}
...
bool Amplitude::deleteLineshapes(){
  bool dbThis=false;
  if(dbThis) cout << "Amplitude::deleteLineshapes()" << endl;
  ...
  if(dbThis) cout << "Amplitude::deleteLineshapes() done" << endl;
  return true;
}
