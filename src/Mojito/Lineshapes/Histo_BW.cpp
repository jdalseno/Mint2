...
double Histo_BW::runningMass2(){
    if(_runningMassHist==0)return mumsMass()*mumsMass();
    
    double d_m2  = _runningMassHist->Interpolate(mumsRecoMass2()/(GeV*GeV));
    double d_m02 = _runningMassHist->Interpolate(mumsMass()*mumsMass()/(GeV*GeV));
    
    return mumsMass()*mumsMass() + mumsWidth()*(d_m2 - d_m02);
}
...
TH1D* Histo_BW::get_width_histo(TFile* f, const std::string& hname){
    TH1D* h=0;
    if(0 != f){
...
TH1D* Histo_BW::get_mass_histo(TFile* f, const std::string& hname){
    TH1D* h=0;
    if(0 != f){
        h = (TH1D*) f->Get(hname.c_str());
        if(0 != h){
            cout << "Found running mass histogram in file " << f->GetName() << "." << endl;
        }else{
            cout << "ERROR: Couldn't find running mass histogram in file " << f->GetName() << ". I'll use a constant mass." << endl;
        }
    }
    else {
        cout << "Didn't find running mass file for " << (BW_BW::resonanceProperties()->nameFromPid(abs(mumsPID()))) << endl;
    }
    return h;
}
